 ...
 * Author: Esther
/*********************************************
 * Base stochastic promotion-replenishment model
 * Thesis replication file
 *
 * Base model:
 * - No emergency trucks
 * - Promotion feasibility through frequency/calendar constraints
 * - Baseline demand uncertainty through scenarios
 * - Optional complementary demand effects
 * - Retailer-funded markdown share and COGS included
 *********************************************/
string runName = ...;
// ---------- Dimensions ----------
...
float dbar[I][T][S] = ...;         // baseline demand
...
float compStrength = ...; // multiplier for complementarity strength
...
// The observed discount depth is the consumer-facing markdown. T
...
//dvar int+ e[T][S];                  // emergency trucks used in period t under scenario s (not used in base case)
...
dexpr float JointEffect[i in I][t in T] =
      useComplementarity * compStrength *
      sum(j in I : j != i) jointGamma[i][j] * bothPromoted[i][j][t];

dexpr float CrossEffect[i in I][t in T] =
      useComplementarity * compStrength *
      sum(j in I : j != i) gamma[i][j] * x[j][t];
...
// Expected full consumer-facing discount value per period. This is not a cost in
// the objective unless retailerPromoShare = 1, but it is useful for reporting.
...
>= minExpectedFillRate *
   ...
   // Only regular trucks are available, because emergency trucks are excluded from the base case.
   forall(t in T, s in S)
   UsedCapacity[t][s] <= Cap * y[t];
  
   // forall(t in T, s in S)									// Emergency trucks are not used in the base model
   ...
   // Promotion-frequency constraints. They prevent promotions from being selected without limit.
   ...
   // Optional minimum number of promotions per product. Set to 0 in the base model, so not active
   forall(i in I)
